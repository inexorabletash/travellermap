...
#Milieu Sector Hex Name UWP TC Remarks Sophonts Details {Ix} (Ex) [Cx] Nobility Bases Zone PBG Allegiance Stars W RU
...
foreach $line (<$in>) {
    ...
    if (!exists $files{$sec}) {
        ...
        $files{$sec} = FileHandle->new("> $dir/../Sectors/M0/$sectors{$sec}.tab");
