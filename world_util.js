...
function worldImageURL(world, type) {
  ...
  switch (type) {
  case 'map':
    return S3_PREFIX + 'maps/' + encodeURIComponent(world.SectorAbbreviation) +
...
const REMARKS_PATTERNS = [
  // Special
  [ /^Rs\w$/, 'Research Station' ], [ /^Rw:?\w$/, 'Refugee World' ],

  // Ownership
  [ /^O:\d\d\d\d$/, 'Controlled' ], [ /^O:\d\d\d\d-\w+$/, 'Controlled' ],
  [ /^O:\w\w$/, 'Controlled' ], [ /^Mr:\d\d\d\d$/, 'Military rule' ],
...
const DETAILS_FETCHED = (async () => {
  const response =
      await fetch(Traveller.MapService.makeURL('/res/maps/world_details.json'));
  if (!response.ok)
    throw Error(response.statusText);
...
function splitPBG(pbg) {
  ...
  return {
    Pop : Util.fromHex(pbg.substring(0, 1)),
    Belts : fix(Util.fromHex(pbg.substring(1, 2)), '???'),
    GG : fix(Util.fromHex(pbg.substring(2, 3)), '???')
  };
};
...
export async function prepareWorld(world) {
  if (!world)
    return undefined;
  ...
  world.PopExp = Util.fromHex(world.UWP.Pop);
  if (world.PopExp > 0 && world.PopMult === 0)
    world.PopMult = 1;
  if (world.PopExp >= 0 && world.PopMult >= 0)
    world.TotalPopulation =
        numberWithCommas(world.PopMult * Math.pow(10, world.PopExp));
  ...
  if (!world.Ix)
    delete world.Ix;
  if (world.Ix) {
    const ix = parseIx(world.Ix);
    world.Ix = {Imp : String(ix)};
  ...
  // Remarks
  if (world.Remarks) {
    world.Remarks = splitRemarks(world.Remarks).map(s => {
      if (s in REMARKS_TABLE)
        return {code : s, detail : REMARKS_TABLE[s]};
      for (let i = 0; i < REMARKS_PATTERNS.length; ++i) {
        const pattern = REMARKS_PATTERNS[i][0],
              replacement = REMARKS_PATTERNS[i][1];
        if (pattern instanceof RegExp && pattern.test(s))
          return {code : s, detail : s.replace(pattern, replacement)};
      }
  ...
  if (world.Remarks) {
    for (const remark of world.Remarks) {
      if ([ 'Re', 'Px', 'Ex' ].includes(remark.code) ||
          remark.code.startsWith('Rs'))
  ...
  world.Stars = world.Stellar.replace(/[OBAFGKM][0-9] D/g, 'D')
                    ...
                    .map(code => {
                      ...
                      for (const key of Object.keys(STELLAR_OVERRIDES)) {
                        if (code.match(STELLAR_OVERRIDES[key]))
                          detail = key;
                      }

                      // Prepend color for ordinary stars
                      for (const key of Object.keys(STELLAR_COLOR)) {
  ...
  world.Zone = (zone => {
    switch (zone) {
    case 'A':
      return {rule : 'Caution', rating : 'Amber', className : 'amber'};
    case 'R':
      return {rule : 'Restricted', rating : 'Red', className : 'red'};
    case 'B':
      return {
    ...
    default:
      return {rule : 'No Restrictions', rating : 'Green', className : 'green'};
    }
  })(world.Zone);
  ...
  if (world.Worlds) {
    ...
    world.OtherWorlds =
        Math.max(world.Worlds - 1 - world.PBG.Belts - world.PBG.GG, 0);
  }
  ...
  world.world_url += `?sector=${encodeURIComponent(world.Sector)}&hex=${
      encodeURIComponent(world.Hex)}`;
  ...
  const map_generator_options = {
    ...
    name : world.Name,
    uwp : world.raw.UWP,
    tc : splitRemarks(world.raw.Remarks),
    iX : parseIx(world.raw.Ix) || '',
    eX : world.raw.Ex || '',
    cX : world.raw.Cx || '',
    pbg : world.raw.PBG,
  ...
  world.map_exists.then(async exists => {
    if (exists) {
      ...
      world.map = map;
      world.map_details =
          (await DETAILS_FETCHED)[world.SectorAbbreviation + ' ' + world.Hex];
    }
  });
...
export async function renderWorldImage(world, canvas) {
  if (!world)
    return undefined;
  ...
  const SIZES = [
    {width : 0.80, height : 0.45}, {width : 0.25, height : 0.25},
    {width : 0.30, height : 0.30}, {width : 0.35, height : 0.35},
    {width : 0.40, height : 0.40}, {width : 0.45, height : 0.45},
    {width : 0.50, height : 0.50}, {width : 0.55, height : 0.55},
    {width : 0.60, height : 0.60}, {width : 0.65, height : 0.65},
    {width : 0.70, height : 0.70}, {width : 0.75, height : 0.75},
    {width : 0.80, height : 0.80}, {width : 0.85, height : 0.85},
    {width : 0.90, height : 0.90}, {width : 0.95, height : 0.95}
  ];
  ...
  try {
    ...
    let m;
    if (!isRender && supportsCompositeMode(ctx, 'destination-in') &&
        supportsCompositeMode(ctx, 'destination-over') &&
        supportsCompositeMode(ctx, 'multiply') && world.Stars &&
        (m = /^([OBAFGKM])([0-9])/.exec(world.Stars[0].code))) {
...
function temp2color(kelvin) {
  ...
  if (kelvin < 6600)
    ...
    r = fit(351.97690566805693, 0.114206453784165, -40.25366309332127,
            (kelvin / 100) - 55);

  if (kelvin <= 1000)
    g = 0;
  else if (kelvin < 6600)
    g = fit(-155.25485562709179, -0.44596950469579133, 104.49216199393888,
            (kelvin / 100) - 2);
  else
    g = fit(325.4494125711974, 0.07943456536662342, -28.0852963507957,
            (kelvin / 100) - 50);

  if (kelvin <= 2000)
    b = 0;
  else if (kelvin < 6600)
    b = fit(-254.76935184120902, 0.8274096064007395, 115.67994401066147,
            (kelvin / 100) - 10);
  else
    b = 255;
