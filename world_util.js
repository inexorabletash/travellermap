...
export async function prepareWorld(world) {
  ...
  if (world.Remarks) {
    world.Remarks = splitRemarks(world.Remarks).map(s => {
      ...
      for (let i = 0; i < REMARKS_PATTERNS.length; ++i) {
        ...
        if (pattern instanceof RegExp && pattern.test(s)) return { code: s, detail: s.replace(pattern, replacement) };
      }
  ...
  world.Zone = (zone => {
    switch (zone) {
      ...
      case 'F': return {
        rule: 'Forbidden', rating: 'c/o Consulate Data Services',
        className: 'forbidden'
      };
      case 'U': return {
        rule: 'Unabsorbed', rating: 'c/o Consulate Data Services',
