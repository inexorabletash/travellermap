...
function parseSector(tabDelimitedData, metadata) {
  ...
  for (const line of lines) {
    ...
    const exp = Util.fromHex(world.uwp.charAt(4));
    const mult = Util.fromHex(world.pbg.charAt(0));
  ...
  let pp = 0;
  for (const page of sector.index_pages) {
    for (const [index, half] of page.entries()) {
      half.pp = ++pp;
    ...
    range(16).forEach(i => {
      ...
      subsector.worlds.forEach(world => {
        ...
        if (world.name !== '') {
          ...
          if (!subsector.maxtl ||
              Util.fromHex(subsector.maxtl[0].uwp.charAt(8)) <
      ...
      pending_promises.push(imageURL.then(url => {
        // This task is run after the page is generated via template.
        return () => fetchImage(
                   url, $(`#ss${subsector.index} img.subsector-image`));
      }));
